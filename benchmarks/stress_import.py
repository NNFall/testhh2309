...
async def snapshot(record_count: int, random_bytes: int, generation: int):
    yield b'{"quotes":[\n'
    buffer = bytearray()
    for number in range(record_count):
        record = {
            "id": f"stress-{number:07d}",
...
async def main(url: str, count: int, random_bytes: int, repeats: int):
    async with aiohttp.ClientSession(base_url=url, timeout=aiohttp.ClientTimeout(total=None)) as session:
        for generation in range(repeats):
            ...
            upload = asyncio.create_task(session.post(
                "/import",
                data=snapshot(count, random_bytes, generation),
        ...
        async with session.get(f"/quotes/stress-{count - 1:07d}") as response:
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.url, args.count, args.random_bytes, args.repeats))
